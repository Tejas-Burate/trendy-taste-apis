...
const createProduct = asyncHandler(async(req,res) => {
    ...
    try {
        ...
        res.status(201).json({status:201, error:"success",message:"Product created successfully"});
    } catch (error) {
...
const getAllProduct = asyncHandler(async(req,res) => {
    try {
        ...
        if(product.length === 0){
            res.status(404).json({status:404, error: "400", message:"Products are not found"});
        }
...
const getProductById = asyncHandler(async(req,res) => {
    try {
        ...
        if(!product){
            res.status(404).json({status:404, error: "400", message: "Product of given Id is not found"});
            return;
        }
        res.status(200).json(product);
...
const getProductsByRestaurantId = asyncHandler(async(req,res) => {
    ...
    if (!isValidObjectId(req.params.id)) {
      ...
      if(!restaurant){
        res.status(404).json({ status: 404, error: "404", message: "Given Restaurant Id is not found" });
        return;
      }

      const product = await Product.find({restaurantId : req.params.id});
      if(product.length === 0){
        res.status(404).json({ status: 404, error: "404", message: "Product of Given Restaurant Id is not found" });
        return;
      }
...
const editProductByProductId = asyncHandler(async(req,res) => {
    const {  productName,
        ...
        try {
            ...
            if(!product){
                res.status(404).json({status: 404, error: "404", message: "Product of given Id is not found"});
                return;
            }

...
try {
    ...
    res.status(201).json({status:201, error:"success", message: "Campus created successfully"});
   } catch (error) {
...
const getAllCampus = asyncHandler(async(req,res) => {
    ...
    if(!campus){
        res.status(404).json({status:404,error:"404",message:"Campus of given ID is not found"});
        return;
    }
...
const getCampusByCampusId = asyncHandler(async(req,res) =>{
   ...
   try{ const campus = await Campus.findOne({_id : req.params.id});
    if(!campus){
        res.status(404).json({status:404, error:"404", message:"Given campus Id is not Found"});
        return ;
    }
    ...
    console.log(error);
    res.status(500).json({ststus:500, error:"500", message:"Internal server error"});
}
});
...
const getCampusByUserId = asyncHandler(async(req,res) => {
   try {
    ...
    if(!user){
        res.status(404).json({status:404, error:"404", message: "Given User Id is not found"});
        return;
    }
     
    const campus = await Campus.find({ createdBy : req.params.id});

    if(campus.length === 0){
        res.status(404).json({status:404, error:"404", message: "Campus of given User Id is not found"});
        return;
    }
...
const editCampusByCampusId = asyncHandler(async(req,res) => {
    ...
    const {campusName,
        ...
        try {
            const campus = await Campus.find({_id : req.params.id});
            console.log('campus', campus)
            ...
            res.status(200).json({status:200, error:"success", message: "Campus data updated successfully"})
        } catch (error) {
            console.log(error);
            res.status(500).json({ status: 500, error: 'Server Error', message: 'An error occurred while processing your request.' });
        }
...
const getDataTableForCampusByuserId = asyncHandler(async (req, res) => {
    ...
    let query = Campus.find(searchQuery)
      .skip(start)
      .limit(length);
